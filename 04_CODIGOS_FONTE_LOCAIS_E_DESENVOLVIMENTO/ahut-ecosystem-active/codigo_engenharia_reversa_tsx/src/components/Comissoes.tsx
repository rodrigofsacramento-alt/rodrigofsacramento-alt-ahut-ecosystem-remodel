  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="glass-neon-card p-5">
          <p className="text-xs text-slate-400 font-bold uppercase">Comissões Pendentes</p>
          <p className="text-2xl font-bold text-white mt-1">{formatCurrency(totalPendente)}</p>
        </div>
        <div className="glass-neon-card p-5">
          <p className="text-xs text-slate-400 font-bold uppercase">Comissões Pagas</p>
          <p className="text-2xl font-bold text-emerald-600 mt-1">{formatCurrency(totalPago)}</p>
        </div>
        <div className="glass-neon-card p-5">
          <div className="flex items-center justify-between">
            <p className="text-xs text-slate-400 font-bold uppercase">Câmbio (USD)</p>
            <button
              onClick={() => setCambio(cambio === 'R$ 5,42' ? 'R$ 5,38' : 'R$ 5,42')}
              className="text-[10px] text-cyan-500 font-bold flex items-center gap-1 hover:underline"
            >
              <RefreshCw className="w-3 h-3" /> Atualizar
            </button>
          </div>
          <p className="text-2xl font-bold text-white mt-1">{cambio}</p>
      ...
      {tab === 'comissoes' ? (
        <div className="glass-neon-card overflow-hidden">
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-white/5 text-left text-xs text-slate-400 uppercase font-bold border-b border-cyan-900/30">
            ...
            <tbody>
              {comissoesMock.map((c) => (
                <tr key={c.id} className="border-b border-white/5 hover:bg-white/50">
                  <td className="px-5 py-3 font-bold text-white">{c.agente}</td>
                  ...
                  <td className="px-5 py-3 font-bold text-cyan-400">{formatCurrency(c.valorComissao)}</td>
                  <td className="px-5 py-3 text-xs text-slate-400">{c.faixa}</td>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="glass-neon-card p-6">
            <h4 className="text-sm font-bold text-white mb-4 flex items-center gap-2"><Award className="w-4 h-4 text-cyan-500" /> Regras de Comissão</h4>
            <div className="space-y-3">
              {faixasComissao.map((f, i) => (
                <div key={i} className="flex items-center justify-between bg-white/5 rounded-xl p-3">
                  <div className="flex items-center gap-2">
                    <div className="w-8 h-8 rounded-lg bg-cyan-500/20 text-cyan-400 flex items-center justify-center text-xs font-bold">{f.pct}%</div>
                    <span className="text-sm text-slate-300">{formatCurrency(f.de)} - {formatCurrency(f.ate)}</span>
                  </div>
          ...
          {/* Split Assessoria */}
          <div className="glass-neon-card p-6">
            <h4 className="text-sm font-bold text-white mb-4 flex items-center gap-2"><Users className="w-4 h-4 text-cyan-500" /> Split de Comissão</h4>
            <div className="space-y-3">
              <div className="flex items-center justify-between bg-white/5 rounded-xl p-3">
                <span className="text-sm text-slate-300">Assessoria Comercial</span>
                <span className="text-sm font-bold text-cyan-400">3%</span>
              </div>
              <div className="flex items-center justify-between bg-white/5 rounded-xl p-3">
                <span className="text-sm text-slate-300">Corretor (Agente / Corretor)</span>
                <span className="text-sm font-bold text-white">5%</span>
              </div>
              <div className="flex items-center justify-between bg-white/5 rounded-xl p-3">
                <span className="text-sm text-slate-300">Bônus Cumulativo não cumulativo</span>
                <span className="text-sm font-bold text-white">+0,5%</span>
              </div>
            </div>
